...
# Opened items are resellable only after handling, so we keep a slice of the price.
...
def refund_amount(
    price: float, days_since_delivery: int, opened: bool = False
) -> float:
    """Return the refund: full price within the window, 0.0 outside it.

    Opened items inside the window refund the price less a 15% restocking fee.
    """
    if price < 0:
